...
/**
 * WhatsAppThread — the hero centerpiece. An authentic, token-only WhatsApp
 * conversation that shows a patient message becoming a *booked, deposit-paid*
 * appointment. Built entirely from the design-system's WhatsApp-chrome tokens
 * (`whatsapp-chrome-fill` teal header, `whatsapp-chrome-canvas` beige surface,
 * `whatsapp-chrome-action` send/pay green) plus product surface tokens — no
 * hardcoded color, no image assets → Server Component.
 *
 * The thread is intentionally bilingual (Arabic ⇄ English) the way GCC patients
 * actually message. Bubbles reveal in a staggered cascade on load (motion-safe
 * only; reduced-motion users get the final layout instantly). A floating
 * revenue card overlaps the frame for depth and to land the "this is money"
 * point. Logical properties throughout, so it mirrors cleanly under RTL.
 */
...
/** A centered system event chip (deposit paid / appointment booked). */
function EventChip({
  children,
  delay,
}: {
...
export function WhatsAppThread({ className }: { className?: string }) {
  return (
    <div className={cn("relative mx-auto w-full max-w-[400px] pb-12 pe-2 sm:pe-10", className)}>
      ...
      <div className="overflow-hidden rounded-[2rem] border border-border-default bg-surface shadow-elev-3">
        ...
        <div className="flex items-center gap-3 bg-whatsapp-chrome-fill px-4 py-3 text-content-inverted">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-content-inverted/15">
            <Icon icon={Stethoscope} size={20} aria-hidden="true" />
          </span>
          <span className="flex min-w-0 flex-1 flex-col leading-tight">
            <span className="truncate text-body-sm font-semibold">Lumière Dental · أزير</span>
        ...
        <div className="flex flex-col gap-3 bg-whatsapp-chrome-canvas px-3.5 py-4">
          <Bubble side="in" time="6:42 PM" delay={0}>
            <span dir="rtl" className="block text-start">
              مرحبا 👋 أبغى موعد تنظيف أسنان هذا الأسبوع
            </span>
          </Bubble>

          <Bubble side="out" time="6:42 PM" read agent delay={220}>
            <span dir="rtl" className="block text-start">
              أهلاً سارة! متوفر لدينا:
              <br />• الثلاثاء 4:00 م — د. لينا
              <br />• الأربعاء 6:30 م — د. لينا
              <br />أي وقت يناسبك؟
            </span>
          </Bubble>

          <Bubble side="in" time="6:44 PM" delay={460}>
            Wednesday 6:30 works 👍
          </Bubble>

          <Bubble side="out" time="6:44 PM" read delay={680}>
            Perfect — to lock in <strong className="font-semibold">Wed 6:30 PM with Dr. Lina</strong>,
            a refundable <strong className="font-semibold">SAR&nbsp;150</strong> deposit secures your slot.
            ...
            <span className="mt-2 flex items-center gap-2 rounded-xl bg-whatsapp-chrome-action px-3 py-2 text-content-inverted">
              ...
              <span className="flex flex-1 flex-col leading-tight">
                <span className="text-body-xs font-semibold">Pay deposit · SAR 150</span>
                <span className="text-body-xs opacity-90">Mada · Apple Pay · Card</span>
              </span>
            </span>
          </Bubble>
          ...
          <EventChip delay={920}>
            ...
            Deposit paid · SAR 150
          </EventChip>
          <EventChip delay={1080}>
            <Icon icon={CheckCheck} size={14} aria-hidden="true" />
            Appointment booked — Wed 6:30 PM
          </EventChip>
        </div>
      </div>
      ...
      <div
        className="absolute bottom-0 end-0 w-56 rounded-2xl border border-border-default bg-surface p-4 shadow-elev-3 motion-safe:animate-in motion-safe:fade-in-0 motion-safe:slide-in-from-bottom-4 motion-safe:duration-700"
      ...
      >
        ...
        <p className="text-body-xs text-fg-muted">from after-hours WhatsApp bookings</p>
        {/* Tiny bar sparkline */}
